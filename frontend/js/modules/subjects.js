...
async function openSubjectDetail(subjectId) {
  ...
  showModal({
    ...
    body: `
      ...
      ${materials.length
          ? `<div style="margin-top:20px;">
              <label class="form-label">\u5df2\u4e0a\u4f20 (${materials.length})</label>
              ${materials.map((m) => `<div class="file-item">
    ...
    footer: `
      ...
      <div class="modal-footer-actions">
        ...
        <button class="btn btn-secondary" id="btnExtract" ${materials.length ? '' : 'disabled'}>\u62bd\u53d6\u4e13\u6709\u540d\u8bcd\u4e0e\u672f\u8bed</button>
  ...
  document.getElementById('btnUpload').onclick = async () => {
    ...
    await api.uploadMaterials(subjectId, pendingFiles);
    showToast('\u8d44\u6599\u4e0a\u4f20\u6210\u529f', 'success');
    document.getElementById('modalOverlay').classList.remove('active');
    renderSubjects(document.getElementById('pageContent'));
  };
  ...
  document.getElementById('btnExtract').onclick = async () => {
    ...
    try {
      const cards = await api.extractConcepts(subjectId, setProgress);
